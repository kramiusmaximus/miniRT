...
static void light_calculations(t_lvars *lvars, t_ray *ray, t_scene *scene)
{
	...
	{
		lvars->pass = lvars->inter_l->obj->transperancy * max_f(pow(1 - lvars->inter_l->ref_coeff, 0.1), 0.7);
		free(lvars->inter_l);
	}
...
void light_effects(t_ray *ray, t_scene *scene, int *c, t_intersect *inter)
{
	...
	while (lvars.p)
	{
		...
		lvars.diff = rgb_add(rgb_multiply_scalar(lvars.light->color, lvars.dot * lvars.light->intensity * lvars.pass), lvars.diff);
		lvars.spec = rgb_add(rgb_multiply_scalar(lvars.light->color, (pow(200, -(200 / lvars.dot - 200))) * lvars.light->intensity * lvars.pass),
							 lvars.spec);
